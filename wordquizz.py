...
class QuizzWindow():   
	...
	def display_word_quizz_question(self): 
		...
		try:
			...
			self.display_word_quizz_lbl.configure(text = f"What does '{question}' mean?", fg = self.std_font_colour)
			self.display_word_quizz_lbl.after(4000, lambda: self.display_word_quizz_answer(answer))

		except: 
			self.display_word_quizz_lbl.configure(text = 'Sorry, something went wrong.', fg = self.std_font_colour)
